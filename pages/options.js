...
function delete_action(id, div) {
  div.fadeOut('swing', function () {
    ...
    let undo = $('<a>undo</a>').click({i: id, param: params.actions[id]}, function (event) {
...
function setup_action(param, id) {
  ...
  let edit = $('<a href=\'#\' class=\'button edit\'>Edit</a>').click({i: id}, function (event) {
    load_action(event.data.i, $(this).parent().parent());
    return false;
  });

  let del = $('<a href=\'#\' class=\'button delete\'>Delete</a>').click({i: id}, function (event) {
...
function setup_form() {
  ...
  for (var i in config.actions) {
    let act = $(
      '<input type="radio" name="action" value="' + i + '" id="form_' + i + '"/>' + config.actions[i].name + '<br/>',
    );
...
function displayOptions(action) {
  ...
  for (let i in config.actions[action].options) {
    ...
    case 'textbox':
      p.append(
        '<input type="text" name="' + op.name + '" id="form_option_' + config.actions[action].options[i] + '"/>',
      );
      break;

    case 'checkbox':
      p.append(
        '<input type="checkbox" name="' + op.name + '" id="form_option_' + config.actions[action].options[i] + '"/>',
      );
      break;
    ...
    case 'selection-textbox':
      ...
      p.append(
        '<input type="text" name="' + op.name + '" id="form_option_text_' + config.actions[action].options[i] + '"/>',
      );
      break;
    }
