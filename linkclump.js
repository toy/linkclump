  ...
  .then(response => {
    if (response === null) {
      ...
      for (let i in response.blocked) {
        ...
        let re = new RegExp(response.blocked[i], 'i');
  ...
  if (allow_selection()) {
    ...
    if (timer !== 0) {
      ...
      if (this.box === null) {
        ...
        count_label.style.textShadow = '1px 0 1px white, 0 1px 1px white, -1px 0 1px white, 0 -1px 1px white';
...
function start() {
  ...
  // create RegExp once
  let re1 = new RegExp('^javascript:', 'i');
  let re2 = new RegExp(this.settings[this.setting].options.ignore.slice(1).join('|'), 'i');
  let re3 = new RegExp('^H\\d$');
  ...
  for (let i = 0; i < page_links.length; i++) {
    ...
    let comp = window.getComputedStyle(page_links[i], null);
    if (comp.visibility == 'hidden' || comp.display == 'none') {
      continue;
    }

    let pos = this.getXY(page_links[i]);
    ...
    page_links[i].important =
      this.settings[this.setting].options.smart == 0 &&
...
function scroll() {
  if (this.allow_selection()) {
    let y = this.mouse_y - window.scrollY;
